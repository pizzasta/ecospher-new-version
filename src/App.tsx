...
import './components/relics-polish.css'
import './components/frequency-polish.css'
import './components/rooms-polish.css'
...
type ReturnRitualState = {
  ...
  daysAway: number
  cameBack: string[]
  ritualTraces: RitualTrace[]
}

type RitualSurface = 'observatory' | 'feed' | 'unsent' | 'drift' | 'chambers' | 'relics'

type RitualRoomModifier = {
  atmosphere: string
  copy: string
  cta: string
...
type SignalRitual = {
  ...
  trace: string
  recap: string
  tone: 'amber' | 'magenta' | 'cyan' | 'graphite'
  audioKey: SampleKind
  modifiers: Partial<Record<RitualSurface, RitualRoomModifier>>
}

type ActiveSignalRitual = SignalRitual & {
  endsLabel: string
  todayKey: string
  windowLabel: string
}
...
const signalRitualDayStorageKey = 'ecosphereSignalRitualDay'
const signalRitualTracesStorageKey = 'ecosphereSignalRitualTraces'

const signalRituals: SignalRitual[] = [
  {
    id: 'amber-return',
    title: 'The Room Remembered',
    body: 'A frequency from last night drifted back. Someone stayed in the room long after the signal ended.',
    cta: 'acknowledge it',
    trace: 'amber return acknowledged',
    recap: 'You stayed with the echo.',
    tone: 'amber',
    audioKey: 'static',
    modifiers: {
      observatory: { atmosphere: 'amber glow', copy: 'The room still carries last night.', cta: 'feel it', surface: 'observatory' },
      feed: { atmosphere: 'late signal', copy: 'Someone replayed your signal at 2am.', cta: 'let it pass', surface: 'feed' },
    },
  },
  {
    id: 'magenta-stranger',
    title: 'Stranger Signal',
    body: 'A voice you never heard before passed through the frequency. It left something behind.',
    cta: 'mark the trace',
    trace: 'stranger signal traced',
    recap: 'The stranger left a mark.',
    tone: 'magenta',
    audioKey: 'whisper',
    modifiers: {
      observatory: { atmosphere: 'foreign frequency', copy: 'A new signal crossed your orbit tonight.', cta: 'trace it', surface: 'observatory' },
      drift: { atmosphere: 'drifting stranger', copy: 'The voice drifted through without a name.', cta: 'let it drift', surface: 'drift' },
    },
  },
  {
    id: 'cyan-revival',
    title: 'Late Night Revival',
    body: 'The frequency came back to life after midnight. These signals only breathe after the city quiets.',
    cta: 'tune in',
    trace: 'late night revival tuned',
    recap: 'You caught the midnight signal.',
    tone: 'cyan',
    audioKey: 'tone',
    modifiers: {
      observatory: { atmosphere: 'midnight frequency', copy: 'The late signal only breathes past midnight.', cta: 'listen now', surface: 'observatory' },
      unsent: { atmosphere: 'unsent at midnight', copy: 'Some signals are only sent when everyone else is asleep.', cta: 'send it', surface: 'unsent' },
    },
  },
  {
    id: 'graphite-silence',
    title: 'Comfortable Silence',
    body: 'Nothing was said. But the frequency stayed open for 40 minutes. That counts.',
    cta: 'sit with it',
    trace: 'silence acknowledged',
    recap: 'Silence was enough.',
    tone: 'graphite',
    audioKey: 'zone',
    modifiers: {
      observatory: { atmosphere: 'held silence', copy: 'Not every signal needs a voice.', cta: 'hold it', surface: 'observatory' },
...
function HomeScreen({ onNavigate }: { onNavigate?: (next: Screen) => void }) {
  ...
  const performSignalRitual = (surface: RitualSurface) => {
    ...
    void playSample(homeAudio, { id: `ritual-${activeRitual.id}`, label: `signal ritual: ${activeRitual.title}`, source: 'home' }, activeRitual.audioKey, getDateSeed(activeRitual.todayKey), 4000)
  }
...
function getRitualNightKey(date = new Date()) {
  const ritualDate = new Date(date)
  if (ritualDate.getHours() < 5) {
    ritualDate.setDate(ritualDate.getDate() - 1)
  }
  return [
    ritualDate.getFullYear(),
    String(ritualDate.getMonth() + 1).padStart(2, '0'),
    String(ritualDate.getDate()).padStart(2, '0'),
  ].join('-')
}

function getRitualWindowLabel(date = new Date()) {
  const hour = date.getHours()
  if (hour >= 23 || hour < 2) return 'midnight window'
  if (hour < 5) return 'late signal window'
  if (hour >= 18) return 'evening return window'
  return 'waiting for tonight'
}

function createActiveSignalRitual(date = new Date()): ActiveSignalRitual {
...
function readRitualTraces(): RitualTrace[] {
  try {
    const parsed = JSON.parse(window.localStorage.getItem(signalRitualTracesStorageKey) ?? '[]')
    if (!Array.isArray(parsed)) return []
    return parsed
      .filter((trace): trace is RitualTrace => (
        trace &&
        typeof trace.id === 'string' &&
        typeof trace.ritualId === 'string' &&
        typeof trace.message === 'string' &&
        typeof trace.recap === 'string' &&
        typeof trace.createdAt === 'string'
      ))
      .slice(0, 8)
  } catch {
    window.localStorage.removeItem(signalRitualTracesStorageKey)
    return []
  }
}

function recordSignalRitualTrace(ritual: ActiveSignalRitual, surface: RitualSurface) {
  const modifier = getRitualModifier(ritual, surface)
  const trace: RitualTrace = {
    id: `${ritual.id}-${Date.now()}`,
    ritualId: ritual.id,
    message: modifier ? `${ritual.trace}: ${modifier.copy}` : ritual.trace,
    recap: ritual.recap,
    createdAt: new Date().toISOString(),
  }
  const traces = [trace, ...readRitualTraces()].slice(0, 8)
  window.localStorage.setItem(signalRitualTracesStorageKey, JSON.stringify(traces))
  return traces
}

function getRitualModifier(ritual: ActiveSignalRitual, surface: RitualSurface) {
  return ritual.modifiers[surface] ?? ritual.modifiers.observatory ?? {
    atmosphere: ritual.title,
    copy: ritual.body,
    cta: ritual.cta,
    surface,
  }
...
function getRitualSurface(navItem: string): RitualSurface {
  if (navItem === 'Signal Stream') return 'feed'
  if (navItem === 'Unsent Room') return 'unsent'
  if (navItem === 'Drift') return 'drift'
  if (navItem === 'Chambers') return 'chambers'
  if (navItem === 'Relics') return 'relics'
  return 'observatory'
}
...
function SignalRitualCard({ onAct, ritual }: { onAct: () => void; ritual: ActiveSignalRitual }) {
  return (
    <article className={`signal-ritual-card ritual-tone-${ritual.tone}`}>
      <div className="ritual-card-traces" aria-hidden="true">
        <i />
        <i />
        <i />
      </div>
      <span>signal ritual</span>
      <h2>{ritual.title}</h2>
      <p>{ritual.body}</p>
      <LpWaveform active bars={12} seed={42} tint="pink" />
      <footer>
        <small>{ritual.windowLabel} / {ritual.endsLabel}</small>
        <button type="button" onClick={onAct}>{ritual.cta}</button>
      </footer>
    </article>
  )
}

function RitualAtmosphere({ children, ritual, surface }: { children: React.ReactNode; ritual: ActiveSignalRitual; surface: RitualSurface }) {
  const modifier = getRitualModifier(ritual, surface)
  return (
    <>
      <div className={`ritual-atmosphere ritual-${ritual.id} ritual-tone-${ritual.tone} ritual-surface-${surface}`} aria-hidden="true">
        <span>{modifier.atmosphere}</span>
        <i />
        <i />
        <i />
      </div>
...
function RitualModifierPanel({
  onAct,
  ritual,
  surface,
}: {
  onAct: (surface: RitualSurface) => void
  ritual: ActiveSignalRitual
  surface: RitualSurface
}) {
  const modifier = getRitualModifier(ritual, surface)
  return (
    <aside className={`ritual-modifier-panel ritual-tone-${ritual.tone}`}>
      <span>{ritual.title}</span>
      <strong>{modifier.atmosphere}</strong>
      <p>{modifier.copy}</p>
      <button type="button" onClick={() => onAct(surface)}>{modifier.cta}</button>
    </aside>
  )
}
...
export default function App() {
  ...
  useEffect(() => {
    if (import.meta.env.PROD && 'serviceWorker' in navigator) {
      ...
      navigator.serviceWorker.register(`/sw.js?v=${encodeURIComponent(import.meta.env.VITE_SW_VERSION ?? 'dev')}`)
