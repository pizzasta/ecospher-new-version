...
ambient: '1800ms cubic-bezier(0.45, 0, 0.2, 1)',
},
surfaces: {
...
const cardClassName = `atmospheric-card ${className}`.trim()
if (as === 'section') return <section className={cardClassName}>{children}</section>
if (as === 'aside') return <aside className={cardClassName}>{children}</aside>
...
return (
<div className={`soft-empty-state ${className}`.trim()}>
<span>{eyebrow}</span>
<p>{message}</p>
</div>
)
...
export function PlayerStateText({
error,
isPlaying,
idle = 'audio preview',
}: {
...
return <small>{error ? 'audio unavailable' : isPlaying ? 'playing softly' : idle}</small>
}
