...
/* ─── per-note layout metadata ─────────────────────────────────────────────── */
interface NoteLayout {
    x: number          // % offset within board
  y: number          // % offset within board
  rot: number        // base rotation degrees
  driftX: number     // ambient drift offset px
  driftY: number     // ambient drift offset px
  driftPhase: number // stagger phase for idle float
  age: number        // 0–1 simulated age factor
  wearLevel: number  // 0–1 surface wear
...
function useAmbientTick() {
    ...
    useEffect(() => {
          ...
          // very slow tick — one frame every ~3 seconds via a timeout relay
                  const slowLoop = () => {
                          setTick(t => t + 1)
                          id = window.setTimeout(slowLoop, 2800 + Math.random() * 1200)
                  }
          id = window.setTimeout(slowLoop, 1500)
          return () => { clearTimeout(id); cancelAnimationFrame(id) }
    }, [])
    return tick
}
...
export default function StickyNotes() {
    ...
    const ambientTick = useAmbientTick()
    const dust = useDustParticles(10)

  // keep layouts in sync when notes change
  useEffect(() => {
        setLayouts(prev => {
  ...
  const onPointerUp = useCallback((id: string) => {
        if (dragging !== id) return
        setDragging(null)
        // soft magnetic snap — nudge toward nearest gentle grid point
                                      setLayouts(prev => {
                                              const m = new Map(prev)
                                              const l = m.get(id)
                                              if (!l) return prev
  ...
  return (
        <section className="drift-section" aria-label="Your notes">
              ...
              <div className="drift-head">
                      <span className="drift-kicker">YOUR NOTES</span>span>
                      <small className="drift-sub">thoughts left behind · drag to place · release one to drift</small>small>
              </div>div>
          ...
          {/* compose */}
              <div className="drift-compose">
                ...
                {error && <span className="drift-error">{error}</span>span>}
              </div>div>
        
          {notes.length === 0 ? (
                  <p className="drift-empty">no notes yet. pin the first thought you don't want to lose.</p>p>
                ) : (
