...
// This is not a social graph. Not a follower list. Not a suggestion engine.
              ...
              <div className="familiar-frequency__strip">
                ...
                {hasMore && (
                    <button
                                ...
                                >
                                <span className="ff-card__handle">+{familiars.length - 4} more</span>span>
                                <span className="ff-card__stage">in the field</span>span>
                    </button>button>
                      )}
              </div>div>
        </div>div>
        ...
        const lastSignalId = f.signalIds[0]
            ...
            const handleClick = () => {
                ...
                const classNames = [
                    ...
                    // stagger entrance so cards don't all land at once
    const style = { animationDelay: `${index * 80}ms` }
        ...
        return (
              <div
                    ...
                    >
                {quiet && <span className="ff-card__quiet-badge">quiet</span>span>}
              
                    <div className="ff-card__handle">{f.handle}</div>div>
                    <div className="ff-card__stage">{stageLabel(f.stage)}</div>div>
                ...
                {f.lastContent && (
                    ...
                    <div className="ff-card__reason">{familiarReason(f)}</div>div>
              </div>div>
            )
}</div>
